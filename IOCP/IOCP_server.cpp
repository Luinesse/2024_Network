...
void WorkerThread(HANDLE iocpHd) {
    ...
    while (TPoolRunning) {
        ...
        if (lpOverlapped == &session->readOverLapped) {
            ...
            session->sendQueue = allMsg;
            session->sendQueue.push(session->buf);
            for(int i = 0; i < session->readMsg; i++) {
                session->sendQueue.pop();
            }

            while(!session->sendQueue.empty()) {
                sendMsg += session->sendQueue.front();
                session->sendQueue.pop();
                session->readMsg++;
            }
            // session->sendQueue.push(session->buf);
            ...
            wsabuf_S.buf = const_cast<char*>(sendMsg.c_str());
            wsabuf_S.len = static_cast<ULONG>(sendMsg.size());
